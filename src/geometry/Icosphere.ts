...
class Icosphere extends Drawable {
  ...
  create() {
    ...
    this.buffer = buffer0;
    var indices = new Uint32Array(this.buffer, indexByteOffset, triangles.length * 3);
    var normals = new Float32Array(this.buffer, normalByteOffset, vertices.length * 4);
    var positions = new Float32Array(this.buffer, positionByteOffset, vertices.length * 4);
